        ...
        try:
            ...
            for account in accounts_data['accounts']:
                if account['username'] == self.account_name:
                    self.username = account['username'].strip()
                    self.email = account['email'].strip()
                    ...
                    # Remove any JSON escape sequences that might have been added
                    if isinstance(raw_password, str):
                        # Handle common escape sequences
                        self.password = raw_password.replace('\\"', '"')
                        self.password = self.password.replace('\\\\', '\\')
                        
                        # Log password length for debugging (without revealing the actual password)
                        print(f"Loaded password for {self.account_name} (length: {len(self.password)})")
                    else:
                        self.password = str(raw_password)
                        print(f"Warning: Password for {self.account_name} was not a string. Converted to string.")
                    
                    return
                    
            raise ValueError(f"Account '{self.account_name}' not found in accounts.json")
            
        except (FileNotFoundError, json.JSONDecodeError) as e:
            raise ValueError(f"Error loading accounts file: {str(e)}")
